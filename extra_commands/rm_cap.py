...
def register_rm_cap(app):
    ...
    @app.on_message(filters.command(["rm_cap"], prefixes=["/"]) & filters.group)
    async def rm_cap_cmd(client, message: Message):
        print("🔥 rm_cap command triggered")  # debug
        ...
        if removed:
            await message.reply(
                f"✅ {rm_captain.first_name} is no longer Team {team} Captain!",
