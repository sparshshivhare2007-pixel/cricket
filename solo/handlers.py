...
async def send_bowling_video(client, chat_id, bowler):
    print(f"🔴 send_bowling_video() - Chat: {chat_id}")
    game = games.get(chat_id)
    if not game or game.get("status") != "playing" or game.get("game_over"):
        print(f"🔴 Cannot send bowling video - game not playing")
        return
    ...
    try:
        ...
        print(f"🔴 Bowling DM sent to user: {bowler['id']}")
    except Exception as e:
        print(f"🔴 Failed to send DM to bowler: {e}")
...
async def bowling_timeout_with_warnings(client, chat_id, user_id, bowler_name, message_id):
    print(f"🔴 bowling_timeout_with_warnings() started for chat: {chat_id}")
...
async def send_bowling_video_team(client, chat_id, bowler):
    print(f"🔴 send_bowling_video_team() - Chat: {chat_id}")
...
🏏 **Enjoy the game!** 🏏"""
    ...
    async def user_info_cmd(client, message: Message):
        ...
        print(f"🔴 USER_INFO - Chat: {message.chat.id}")
...
─────⊱◈◈◈⊰─────"""
    ...
    async def user_ranks_cmd(client, message: Message):
        from pyrogram.enums import ParseMode
        from database import get_or_create_user
        print(f"🔴 USER_RANKS - Chat: {message.chat.id}")
...
🧑‍✈️ Best Game Host: {best_game_host}"""
    ...
    async def member_lists_cmd(client, message: Message):
        chat_id = message.chat.id
        print(f"🔴 MEMBER_LISTS - Chat: {chat_id}")
    ...
    async def startgame_cmd(client, message: Message):
        chat_id = message.chat.id
        user_id = message.from_user.id
        print(f"🔴 STARTGAME - Chat: {chat_id}, User: {user_id}")
    ...
    async def live_matches_cmd(client, message: Message):
        chat_id = message.chat.id
        print(f"🔴 LIVE_MATCHES - Chat: {chat_id}")
    ...
    async def host_change_cmd(client, message: Message):
        chat_id = message.chat.id
        user_id = message.from_user.id
        print(f"🔴 HOST_CHANGE - Chat: {chat_id}, User: {user_id}")
    ...
    async def solo_leave_cmd(client, message: Message):
        chat_id = message.chat.id
        user_id = message.from_user.id
        print(f"🔴 SOLO_LEAVE - Chat: {chat_id}, User: {user_id}")
    ...
    async def full_score_cmd(client, message: Message):
        chat_id = message.chat.id
        print(f"🔴 FULL_SCORE - Chat: {chat_id}")
    ...
    async def report_user_cmd(client, message: Message):
        chat_id = message.chat.id
        user_id = message.from_user.id
        print(f"🔴 REPORT_USER - Chat: {chat_id}")
    ...
    async def report_stats_cmd(client, message: Message):
        chat_id = message.chat.id
        user_id = message.from_user.id
        print(f"🔴 REPORT_STATS - Chat: {chat_id}")
    ...
    async def cap_change_cmd(client, message: Message):
        chat_id = message.chat.id
        user_id = message.from_user.id
        print(f"🔴 CAP_CHANGE - Chat: {chat_id}")
    ...
    async def end_match_cmd(client, message: Message):
        chat_id = message.chat.id
        user_id = message.from_user.id
        print(f"🔴 END_MATCH - Chat: {chat_id}")
    ...
    async def end_solo_confirm_callback(client, callback):
        ...
        print(f"🔴 END_SOLO_CONFIRM - Chat: {chat_id}")
    ...
    async def end_team_confirm_callback(client, callback):
        chat_id = int(callback.data.split("_")[3])
        user_id = callback.from_user.id
        print(f"🔴 END_TEAM_CONFIRM - Chat: {chat_id}")
    ...
    async def bowling_dm(client, message):
        ...
        num = int(text)
        print(f"🔴 BOWLING_DM - User: {user_id}, Number: {num}")
    ...
    async def batting_msg(client, message):
        ...
        if game:
            ...
            bat = int(text)
            print(f"🔴 BATTING SOLO - Chat: {chat_id}, Batter: {batter['name']}, Number: {bat}")
        ...
        print(f"🔴 BATTING TEAM - Chat: {chat_id}, Team: {team_game['current_team']}, Batter: {batter['name']}, Bat: {bat}, Bow: {bow}")
    ...
    async def mode_handler(client, callback: CallbackQuery):
        action = callback.data.split("_")[1]
        print(f"🔴 MODE_HANDLER - Action: {action}")
    ...
    async def ball_handler(client, callback: CallbackQuery):
        ...
        print(f"🔴 BALL_HANDLER - Ball mode: {ball_mode}, Chat: {chat_id}")
    ...
    async def start_game_match(client, chat_id):
        ...
        result = start_match(chat_id)
        print(f"🔴 start_match result: {result}")
        ...
        print(f"🔴 ========== START GAME MATCH END ==========")

    # ================= TEAM MODE START =================
    async def team_mode_start(client, callback):
        chat_id = callback.message.chat.id
        print(f"🔴 TEAM_MODE_START - Chat: {chat_id}")
    ...
    async def team_b_timer(client, chat_id):
        ...
        if game and game.get("status") == "team_creation_b":
            ...
            await client.send_message(chat_id, f"⏰ Time's up for Team B! ({team_b_count} players joined)\n\n👥 Final Teams:\n🏏 Team A: {team_a_count} players\n🏏 Team B: {team_b_count} players\n\n🏏 Game is ready to start!")
    ...
    async def vote_system(client, message):
        chat_id = message.chat.id
        print(f"🔴 VOTE_SYSTEM - Chat: {chat_id}")
    ...
    async def add_to_team_a_cmd(client, message: Message):
        chat_id = message.chat.id
        user_id = message.from_user.id
        print(f"🔴 ADD_TO_TEAM_A - Chat: {chat_id}")
        ...
        game = team_games.get(chat_id)
        if not game or game.get("status") not in ["team_creation_a", "team_creation_b", "ready"]:
        ...
        await message.reply(f"added {username_display} to Team A! ({len(game['team_a'])} players)")
    ...
    async def add_to_team_b_cmd(client, message: Message):
        chat_id = message.chat.id
        user_id = message.from_user.id
        print(f"🔴 ADD_TO_TEAM_B - Chat: {chat_id}")
        ...
        game = team_games.get(chat_id)
        if not game or game.get("status") not in ["team_creation_b", "ready"]:
        ...
        game["team_b"].append(player_data)
        username_display = f"@{added_user.username}" if added_user.username else added_user.first_name
        await message.reply(f"added {username_display} to Team B! ({len(game['team_b'])} players)")
    ...
    async def shift_team_cmd(client, message: Message):
        chat_id = message.chat.id
        user_id = message.from_user.id
        print(f"🔴 SHIFT_TEAM - Chat: {chat_id}")
        ...
        in_team_a = any(p["id"] == user_to_shift.id for p in game.get("team_a", []))
        
        if not in_team_a:
            in_team_b = any(p["id"] == user_to_shift.id for p in game.get("team_b", []))
            if not in_team_b:
    ...
    async def shift_confirm_callback(client, callback):
        chat_id = callback.message.chat.id
        user_id = callback.from_user.id
        print(f"🔴 SHIFT_CONFIRM - Chat: {chat_id}")
