# solo/game.py - Updated with Ball Counter
...
def create_game(chat_id):
    games[chat_id] = {
        ...
        "game_over": False,
        "winner": None,
        "current_bowler_balls": 0,  # Balls bowled by current bowler
        "total_balls_in_match": 0,   # Total balls played
...
def start_match(chat_id):
    ...
    ball_mode = game.get("ball_mode", 3)  # Default 3 balls
    
    if len(players) > 0:
        game["current_batter"] = players[0].copy()
        game["current_batter_index"] = 0
        
        # Bowler is next player (index 1) or back to 0 if only 1 player
...
def play_ball(chat_id, bat_number):
    ...
    # Check if game over (all batters out or all bowlers done?)
...
def change_bowler(game):
    ...
    # Find next bowler (not out, different from current)
    ...
    game["current_bowler_balls"] = 0  # Reset ball count for new bowler
    
    return next_bowler_index
...
def check_game_over(game):
    ...
    # Check if all bowlers have bowled their overs
    # For now, game continues until all batters out
    # You can add max balls limit here if needed
